...
public class OutStockPan extends JPanel {
    ...
    //为了方便后面调用这些变量，所以这么定义，而不是设置为局部变量（包括：供应商，商品名称，商品数量，商品价格，客户）
    public static JComboBox cmbSupName;
    public static JComboBox cmbStockName;
    public static JTextField stockNumOut; //为了与入库界面的stockNumIn变量进行区分，方便以后调用，故命名为stockNumOut
    public static JTextField stockPriceOut; //同上
    public static JTextField stockUser;

    public OutStockPan(int x, int y, int width, int height) {//第一，二个参数表示起始位置；第三、四个参数表示方框的宽度与高度
    ...
    void init() {
        ...
        //产品出库信息：商品名称，商品出库时间，商品出库价格，商品出库数量，商品库存，商品供应商，客户名称
        ...
        //添加4个标签，2个文本框，2个下拉菜单
        // 先设置方框二的属性
        jpanel2.setLayout(new FlowLayout(FlowLayout.LEFT, 15, 15)); //左对齐
        ...
        jpanel2.add(stockNumOut);

        JLabel jl4 = new JLabel("商品价格");
        jpanel2.add(jl4);
        stockPriceOut = new JTextField(8);
        jpanel2.add(stockPriceOut);
